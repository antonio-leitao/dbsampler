...
- Starts from a user-defined number of points. More points give broader coverage, while fewer points run faster.
...
- `sparse`: if `True` (default), keeps only the first converged point generated by each pair of neighboring data points.
...
Passing `sparse=True` keeps one representative for each distinct boundary region found by the sampler.
This can drastically reduce the number of returned points while preserving the discovered boundary structure.
